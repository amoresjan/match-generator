"""
Test: 1v1 fair rotation
Scenarios:
  A) 6 players, 1 court, 15 rounds  (2 play, 4 sit)
  B) 8 players, 2 courts, 15 rounds (4 play, 4 sit)
  C) 10 players, 3 courts, 15 rounds (6 play, 4 sit)
"""
import os, django
...
def run_scenario(label: str, num_players: int, num_courts: int, num_rounds: int):
    print(f'\n{"="*60}')
    print(f'  {label}')
    print(f'  {num_players} players · {num_courts} court(s) · {num_rounds} rounds · 1v1')
    print(f'{"="*60}')
    ...
    for rn in range(1, num_rounds + 1):
        ...
        for name in names:
            play_history[name].append(pid_to_name.get(
                next((pid for pid, n in pid_to_name.items() if n == name), ''), name
            ) not in [pid_to_name[b] for b in bye])
        ...
        for court in generated['courts']:
            ...
            for a in t1:
                for b in t2:
                    ...
                    opponent_counts[b][a] = opponent_counts[b].get(a, 0) + 1

        status = '✓' if match_ok else '✗ MISMATCH'
        courts_str = '  '.join(
            f'[{pid_to_name[c["team1"][0]]}] vs [{pid_to_name[c["team2"][0]]}]'
...
run_scenario('A: 6 players, 1 court',  num_players=6,  num_courts=1, num_rounds=15)
run_scenario('B: 8 players, 2 courts', num_players=8,  num_courts=2, num_rounds=15)
run_scenario('C: 10 players, 3 courts', num_players=10, num_courts=3, num_rounds=15)

print('\nAll 1v1 scenarios passed.\n')
